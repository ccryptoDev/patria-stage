<template>
  <div>
    ...
    <button class="btn btn-primary" @click="showModal = !showModal">Add Loan</button>
    <modal :showModal="showModal" :fitContent="page === 2 ? false : true">
      <loan-form
        v-if="page === 1"
        :user="user"
...
export default {
  name: "AdminAddLoan",
  components: { LoanForm, Modal, Offers },
  props: {
    user: {
      type: Object,
      required: true
  ...
  mounted() {
    console.log(this.maximumrequestedloanamount);
  },
  data() {
    return {
      applicationId: null,
